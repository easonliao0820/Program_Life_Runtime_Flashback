...
class SceneTree(py4godot_mainloop.MainLoop):
  ...
  @functools.native_method
  def change_scene_to_node(self, node:'py4godot_node.Node'   ) -> typing.Union[int]:
    r'''
				Changes the running scene to the provided `Node`. Useful when you want to set up the new scene before changing.
				Returns `constant OK` on success, `constant ERR_INVALID_PARAMETER` if the `node` is `null`, or `constant ERR_UNCONFIGURED` if the `node` is already inside the scene tree.
    ...
    _ret:int
    _ret = self._ptr.call_with_return(93,tuple([node._ptr]))
    return _ret
  ...
  def set_multiplayer(self, multiplayer:'py4godot_multiplayerapi.MultiplayerAPI'   , root_path:'object' = None  ) -> None:
    ...
    self._ptr.call_with_return(96,tuple([multiplayer._ptr, py_nodepath_root_path._ptr]))

  @functools.native_method
  def get_multiplayer(self, for_path:'object' = None  ) -> typing.Union['py4godot_multiplayerapiextension.MultiplayerAPIExtension','py4godot_scenemultiplayer.SceneMultiplayer','py4godot_multiplayerapi.MultiplayerAPI']:
    r'''
    ...
    _ret._ptr = self._ptr.call_with_return(97,tuple([py_nodepath_for_path._ptr]))
    return smart_cast(_ret) if not _ret._ptr.is_null() else None
  ...
  def set_multiplayer_poll_enabled(self, enabled:'bool'   ) -> None:
    ...
    self._ptr.call_with_return(98,tuple([enabled]))
