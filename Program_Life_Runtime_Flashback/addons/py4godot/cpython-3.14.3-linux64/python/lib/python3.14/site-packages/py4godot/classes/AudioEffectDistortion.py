...
class AudioEffectDistortion(py4godot_audioeffect.AudioEffect):
  r'''
		...
		**Note:** In a nonlinear function, an input sample at _x_ amplitude value, will either have its amplitude increased or decreased to a _y_ value, based on the function value at _x_, which is why even at the same `drive`, the output sound will vary depending on the input's volume. To change the volume while maintaining the output waveform, use `post_gain`.
